...
import { getLocalStorageItem, ErrorToast, url } from "../../utils/helper";
import {
  ...
  getEditorAdditionMedia,
  getEditorUploadMedia,
} from "../../redux/action";
import leftArrowIcon from "../../assets/images/leftArrowIcon.svg";
import Header from "../../component/layout/Header";
import SliderModal from "./../JobModule/SliderModal";
...
const JobDetail = () => {
  ...
  const [tabViews, setTabViews] = useState([
    { title: "post_production_brief", isActive: false, isActiveGreen: false },
    { title: "raw_media", isActive: false, isActiveGreen: false },
    { title: "edited_media", isActive: false, isActiveGreen: false },
    {
  ...
  const [assigningMediaLoader, setAssigningMediaLoader] = useState(false);
  const [additionalMediaLoader, setAdditionalMediaLoader] = useState(false);
  ...
  const [uppyAnchorRawMediaFlag, setUppyAnchorRawMediaFlag] = useState(false);

  const [jobDetailCount, setJobDetailCount] = useState(0);
  const [imageArray, setImageArray] = useState([]);
  const [imageArrayAssigningMedia, setImageArrayAssigningMedia] = useState([]);
  const [imageArrayAdditionalMedia, setImageArrayAdditionalMedia] = useState(
    []
  );
  const [imageArrayUploadMedia, setImageArrayUploadMedia] = useState([]);
  const [imageArrayUploadMediaTemporary, setImageArrayUploadMediaTemporary] = useState([]);
  ...
  const [invoiceButtonDisableFlag, setInvoiceButtonDisableFlag] =
    ...
    const [editorUpdateApiCallOnce, setEditorUpdateApiCallOnce] = useState(true);

  const [apiCallOnce, setApiCallOnce] = useState(true);

  const [totalImagesEditorAssign, setTotalImagesEditorAssign] = useState(0);
  const [totalImagesEditorAdditional, settotalImagesEditorAdditional] =
    useState(0);
  const [totalImagesEditorUpload, setTotalImagesEditorUpload] = useState(0);
  ...
  const job_detail = useSelector(
    (state) => state?.GetJobDetailEditor?.jobDetailEditor
  );

  var fileExtensionVideo = ["MP4", "mp4", "MOV", "mov"];
  ...
  useEffect(async () => {
    if (user_profile_id && job_detail_id) {
      if (apiCallOnce) {
        defaultApiCall();
      }
    }
  }, [jobDetail?.in_process, jobDetailCount]);
  ...
  useEffect(() => {
    if (getLocalStorageItem("token") && getLocalStorageItem("userData")) {
      if (userData?.roles[0]?.id === 5) {
        ...
        if (userData?.is_profile_completed === 1) {
          history.push(`/job-detail-editor/${job_detail_id}`);
        } else {
...
console.log("scrollTab",scrollTab)
  ...
  const onHandleFetchMoreImages = () => {
    if (scrollTab == 1 && scrollTab != "") {
    
      setAssigningMediaLoader(true);
      let getEditorAssigningMediaPayload = {
        editor_id: user_profile_id,
        job_id: parseInt(job_detail_id),
        page: currPage,
      };
      dispatch(
        getEditorAssigningMedia({
          getEditorAssigningMediaPayload,
          callback: async(result) => {
            if (result) {
        
              setAssigningMediaLoader(false);
              setLastList(result.has_more);
              setPrevPage(currPage);
              totalImagesEditorAssign(result?.total);
              
              const arrayMap = result?.data?.map((ele) => ele?.image_name);
              const removeDuplicate = [...imageArrayAssigningMedia, ...arrayMap];
              const removeDuplicateImageName = [...new Set(removeDuplicate)];
          
              setImageArrayAssigningMedia(removeDuplicateImageName);
              // setImageProductionAdditionalArray([...imageProductionAdditionalArray,...arrayMap])
            }
          },
        })
      );
      ...
      setAdditionalMediaLoader(true);;
      let getEditorAdditionalMediaPayload = {
        job_id: parseInt(job_detail_id),
        page: currPage,
      };
      dispatch(
        getEditorAdditionMedia({
          getEditorAdditionalMediaPayload,
          callback: async (result) => {
            if (result) {
             
              setAdditionalMediaLoader(false)
              setLastList(result.has_more);
              setPrevPage(currPage);
              settotalImagesEditorAdditional(result?.total);

              const arrayMap = result?.data?.map(
                (ele) => ele?.extra_image_name
              );
              const removeDuplicate = [...imageArrayAdditionalMedia, ...arrayMap];
              const removeDuplicateImageName = [...new Set(removeDuplicate)];

              setImageArrayAdditionalMedia(removeDuplicateImageName);
            }
          },
        })
      );
      ...
      setMediaUploadLoader(true);
      let getEditorMediaUploadPayload = {
        editor_id: user_profile_id,
        job_id: parseInt(job_detail_id),
        page: currPage,
      };
      dispatch(
        getEditorUploadMedia({
          getEditorMediaUploadPayload,
          callback: async (result) => {
            if (result) {
              setMediaUploadLoader(false);
              setLastList(result.has_more);
              setPrevPage(currPage);
              setTotalImagesEditorUpload(result?.total);

              const arrayMap = result?.data?.map((ele) => ele?.image_name);

              const removeDuplicate = [...imageArrayUploadMedia, ...arrayMap];
              const removeDuplicateImageName = [...new Set(removeDuplicate)];

              setImageArrayUploadMedia(removeDuplicateImageName);
            }
          },
        })
      );
  ...
  const defaultApiCall = () => {
    setApiCallOnce(false);

    //Editor status update
    if(editorUpdateApiCallOnce){
      setEditorUpdateApiCallOnce(false);
      let editorUpdateStatus = {
        editor_status: 1,
        job_id: parseInt(job_detail_id),
      };
                ...
                getJobDetailEditor({
                  ...
                  callback: async (data) => {
                    if (data) {
                      ...
                      if (data?.code === 200) {
                        if (data?.data[0]?.editor_id !== user_profile_id) {
                          toast.error(<ErrorToast msg="Jod id doesn't exist." />);
                          history.push({
                            pathname: "/dashboard",
                          });
                        } else {
                          setJobDetail(data?.data[0]);
                          if (data?.data[0]?.in_process === 5) {
                          ...
                          } else if (data?.data[0]?.in_process !== 5) {
                            const filterTabList = tabViews.map((el) =>
                              el.title === "raw_media"
                                ? {
                                    ...el,
      ...
      //get images of Editor Assigning Media
      let getEditorAssigningMediaPayload = {
        editor_id: user_profile_id,
        job_id: parseInt(job_detail_id),
        page: currPage,
        ...
        getEditorAssigningMedia({
          ...
          callback: async (result) => {
            if (result) {
              setLastList(result.has_more);
              setPrevPage(currPage);
              setTotalImagesEditorAssign(result?.total);
  
              const arrayMap = result?.data?.map((ele) => ele?.image_name);
              setImageArrayAssigningMedia([
                ...imageArrayAssigningMedia,
                ...arrayMap,
              ]);
              // setImageProductionAdditionalArray([...imageProductionAdditionalArray,...arrayMap])
            }
      ...
      //get images of Editor Additional Media
      let getEditorAdditionalMediaPayload = {
        job_id: parseInt(job_detail_id),
        page: currPage,
      };
        ...
        getEditorAdditionMedia({
          ...
          callback: async (result) => {
            if (result) {
              ...
              const arrayMap = result?.data?.map((ele) => ele?.extra_image_name);
              setImageArrayAdditionalMedia([
                ...imageArrayAdditionalMedia,
                ...arrayMap,
              ]);
            }
          },
      ...
      //get images of Editor Media upload
      let getEditorMediaUploadPayload = {
        editor_id: user_profile_id,
        job_id: parseInt(job_detail_id),
        page: currPage,
        ...
        getEditorUploadMedia({
          ...
          callback: async (result) => {
            if (result) {
              ...
              const arrayMap = result?.data?.map((ele) => ele?.image_name);
              setImageArrayUploadMedia([...imageArrayUploadMedia, ...arrayMap]);
              setImageArrayUploadMediaTemporary([...imageArrayUploadMedia, ...arrayMap])
            }
          },
        })
      );
  ...
  const uppy = useUppy(() => {
    return new Uppy({
      ...
      restrictions: {
        allowedFileTypes: [".jpeg", ".jpg", ".mp4", ".mov", ".png", ".tiff"],
      },
  ...
  const tabSwitch = (e, tab) => {
    ...
    // setImageArrayAssigningMedia([]);
    // setImageArrayAdditionalMedia([]);
    // setImageArrayUploadMedia([]);
    defaultApiCall();
    if (jobDetail?.in_process !== undefined) {
      const filterTabList = tabViews.map((el) =>
        el.title === tab
          ? { ...el, isActive: true }
          : { ...el, isActive: false }
  ...
  const AvatarPicker = () => {
    return (
      <div className={`${fileFlag ? "custom-cancel" : ""}`} ref={mountedRef1}>
        {fileFlag ? (
          <button
            onClick={(e) => {
              handleCancel(e);
          ...
          locale={{
            strings: {
              ...
              dropPasteFiles:
                ...
                "%{browse}" +
                ` ${t("FILE_ACCEPTED_MESSAGE_EDITOR")}`,
            ...
            if (data?.data[0]?.in_process === 5) {
              ...
              const filterTabList = tabViews.map((el) =>
                el.title === "raw_media"
                  ...
                  : { ...el, isActiveGreen: false, isActive: false }
              );
              setTabViews(filterTabList);
            }
          }
        ...
        callback: async (data) => {
          if (data) {
            ...
            if (data?.in_process === 5) {
              ...
              setTabViews(filterTabList);
            } else if (data?.in_process !== 5) {
              const filterTabList = tabViews.map((el) =>
                el.title === "raw_media"
                  ? { ...el, isActiveGreen: true, isActive: true }
                  : { ...el, isActiveGreen: false, isActiveGreen: false }
              );
              setTabViews(filterTabList);
            }
          }
  ...
  const renderDownLoadButton1 = () => {
    return (
      <>
        <div
          disabled={onClickButtonToggle}
          onClick={(e) => downloadImages(e, true)}
          className={`${
            onClickButtonToggle ? "disable-btn-one" : ""
          } download-btn download-btn-height `}
        >
  ...
  const downloadImages = async (e, additional) => {
    e.preventDefault();
          ...
          if (!result) {
            if (data) {
              ...
              if (data?.key === 1) {
                ...
                setImageLinkMessage(
                  "You will soon receive an eamil with download link"
                );
              }
            }
          } else {
  ...
  const handleImage = (image, extenstion) => {
    if (extenstion === "pdf") {
      ...
      return (
        <a target="_blank" title="XMP Caption">
          <svg
            width="20"
            height="25"
            viewBox="0 0 20 25"
      ...
      return (
        <a
          rel="prettyPhotoPostProductionSummary[photography]"
          title="Image Caption"
        >
          <img
            id="images"
            src={`${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_extra/thumbnail/${image}`}
          />
        </a>
      );
    }
  ...
  const handleClassEditedMedia = (extenstion) => {
    if (extenstion === "tiff") {
      return "photography media-svg-icon";
    } else {
      return "photography";
  ...
  const handleImageEditedMedia = (image, extenstion) => {
    if (extenstion === "tiff") {
      return (
        <a target="_blank" title="Pdf Caption">
          <svg
          ...
          >
            ...
            />
            <path
              d="M5.43338 16.6013V17.5133H6.05738V18.0233H5.43338V19.5233C5.43338 19.6593 5.46338 19.7593 5.52338 19.8233C5.58738 19.8873 5.69138 19.9193 5.83538 19.9193C5.91938 19.9193 5.99338 19.9113 6.05738 19.8953V20.3753C5.96138 20.4113 5.83338 20.4293 5.67338 20.4293C5.42538 20.4293 5.22938 20.3573 5.08538 20.2133C4.94138 20.0693 4.86938 19.8673 4.86938 19.6073V18.0233H4.31738V17.5133H4.47338C4.62138 17.5133 4.73138 17.4733 4.80338 17.3933C4.87938 17.3093 4.91738 17.2013 4.91738 17.0693V16.6013H5.43338Z"
              fill="#353535"
            />
            <path
              d="M7.3682 20.3993H6.8102V17.5133H7.3682V20.3993ZM6.7982 16.7093C6.7222 16.6293 6.6842 16.5333 6.6842 16.4213C6.6842 16.3093 6.7222 16.2133 6.7982 16.1333C6.8782 16.0533 6.9742 16.0133 7.0862 16.0133C7.1982 16.0133 7.2942 16.0533 7.3742 16.1333C7.4542 16.2093 7.4942 16.3053 7.4942 16.4213C7.4942 16.5333 7.4542 16.6293 7.3742 16.7093C7.2942 16.7853 7.1982 16.8233 7.0862 16.8233C6.9742 16.8233 6.8782 16.7853 6.7982 16.7093Z"
              fill="#353535"
            />
            <path
              d="M11.4358 16.5653C11.3118 16.5653 11.2038 16.6033 11.1118 16.6793C11.0238 16.7513 10.9798 16.8773 10.9798 17.0573V17.5133H11.6758V18.0233H10.9798V20.3993H10.3978V18.0233H9.08377V20.3993H8.50177V18.0233H7.96777V17.5133H8.50177V17.0273C8.50177 16.7193 8.59177 16.4773 8.77177 16.3013C8.95177 16.1253 9.17777 16.0373 9.44977 16.0373C9.62177 16.0373 9.73377 16.0573 9.78577 16.0973V16.6013C9.72577 16.5773 9.64377 16.5653 9.53977 16.5653C9.41577 16.5653 9.30777 16.6033 9.21577 16.6793C9.12777 16.7513 9.08377 16.8773 9.08377 17.0573V17.5133H10.3978V17.0273C10.3978 16.7193 10.4878 16.4773 10.6678 16.3013C10.8478 16.1253 11.0738 16.0373 11.3458 16.0373C11.5178 16.0373 11.6298 16.0573 11.6818 16.0973V16.6013C11.6218 16.5773 11.5398 16.5653 11.4358 16.5653Z"
              fill="#353535"
            />
            <path
              d="M14.5 13C13.1979 13 5.74154 13 4.43945 13C3.85352 13 3.85352 13.8464 4.43945 13.8464C5.74154 13.8464 13.1979 13.8464 14.5 13.8464C15.0859 13.8464 15.0859 13 14.5 13Z"
              fill="#FF7330"
            />
            <path
              d="M14.5 11C13.1979 11 5.74154 11 4.43945 11C3.85352 11 3.85352 11.8464 4.43945 11.8464C5.74154 11.8464 13.1979 11.8464 14.5 11.8464C15.0859 11.8464 15.0859 11 14.5 11Z"
              fill="#FF7330"
            />
            <path
              d="M14.5 9C13.1979 9 5.74154 9 4.43945 9C3.85352 9 3.85352 9.84635 4.43945 9.84635C5.74154 9.84635 13.1979 9.84635 14.5 9.84635C15.0859 9.84635 15.0859 9 14.5 9Z"
              fill="#FF7330"
            />
          </svg>
        </a>
      ...
      return (
        <a rel="prettyPhotoEditedMedia[photography]" title="Image Caption">
          <img
            id="images"
            src={`${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_${user_profile_id}/thumbnail/${image}`}
          />
        </a>
      );
    }
    ...
    <>
      ...
      <section className="dashboard-main-one position-relative file-set job-detail dashboard-job">
        ...
        <div className="content dashboard-background">
          <img
            onClick={() => history.push("/editor-dashboard")}
            src={leftArrowIcon}
                      ...
                      >
                        ...
                        >
                          ...
                          <h5>
                            ...
                            {imageArrayAssigningMedia?.length} {t("MEDIA")})
                          </h5>
                          <div className="card_detail_label">
                            <span className="category-data">
                              {jobDetail?.project_details?.category_name}
                            </span>
                          </div>
                        </div>
                        <div className="tags_heading">
                          <div className="d-flex align-items-start">
                            <div className="profile_name">
                              ...
                              {jobDetail?.customer_details?.profile_image ? (
                                <img
                                  src={`${process.env.REACT_APP_AWS_DSP_AWS_URL}profile_${jobDetail?.customer_details?.producer_id}/${jobDetail?.customer_details?.profile_image}`}
                                  alt="profile"
                                />
                              ) : (
                                <span>
                                  {
                                    jobDetail?.customer_details?.name?.split(
                                      ""
                                    )[0]
                                  }
                                </span>
                            ...
                            <div className="profile_detail ms-3">
                              <h2 className="">
                                {" "}
                                {jobDetail?.customer_details?.name}
                              </h2>
                              <div className="d-flex mt-1">
                                ...
                                <a
                                  ...
                                  href={`mailto:${jobDetail?.customer_details?.Email}`}
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="14"
                      ...
                      <div className="d-flex justify-content-center preset_tab ">
                        <nav>
                          ...
                          >
                            ...
                            <li
                              onClick={(e) => {
                                tabSwitch(e, "post_production_brief");
                                setScrollTab("2");
                              }}
                              className="nav-item"
                              role="presentation"
                            >
                              <a
                                ...
                                aria-selected="false"
                              >
                                {t("POST_PRODUCTION_BRIEF")}
                              </a>
                            </li>
                            {/*RAW MEDIA */}
                            <li
                              onClick={(e) => {
                                tabSwitch(e, "raw_media");
                                setScrollTab("1");
                              }}
                              className="nav-item"
                              role="presentation"
                        ...
                        <div className="tab-content" id="myTabContent">
                                  ...
                                  {userData?.editing_suite === "1" ? (
                                    ...
                                    >
                                      <a
                                        ...
                                        id="toggle-tab-photo"
                                        data-bs-toggle="tab"
                                        data-bs-target="#toggle-photo"
                                        type="button"
                                        role="tab"
                                        aria-controls="toggle-tab-pane"
                                        aria-selected="false"
                                      ></a>
                                    </li>
                                  ) : userData?.editing_suite === "2" ? (
                                    <li
                                      className="nav-item"
                                      role="presentation"
                                    >
                              ...
                              <div className="tab-content" id="myTabContent">
                                ...
                                <div
                                  className={`tab-pane fade ${
                                    userData?.editing_suite === "1"
                                      ? "show active"
                                      : userData?.editing_suite === "3"
                                      ? "show active"
                                      : ""
                                  }`}
                                  id="toggle-photo"
                                  role="tabpanel"
                                  aria-labelledby="toggle-tab-photo"
                                >
                                  <div className="card-block">
                                    <div className="d-flex card-middle-row ">
                                      <div className="user-card-details d-flex">
                                        <div className="form-switch-cst">
                                          <input
                                            type="checkbox"
                                            className="toggle-button"
                                            id="switch1"
                                            checked={
                                              jobDetail?.post_production
                                                ?.hdr_bracketing || false
                                            }
                                            disabled
                                          />
                                          <label
                                            className="text-black fw-bold ms-2 d-inline-block fs-6"
                                            htmlFor="switch1"
                                          >
                                            {t("HDB_BRACKETING")}
                                          </label>
                                        ...
                                        <div className="form-switch-cst">
                                          ...
                                          >
                                            {t(
                                              "VERTICAL_&_HORIZONTAL STRAIGHTENING"
                                            )}
                                          </label>
                                        </div>
                                        <div className="form-switch-cst">
                                        ...
                                        <div className="form-switch-cst">
                                          ...
                                          >
                                            {t(
                                              "BRIGHTNESS_& CONTRAST_ADJUSTMENT"
                                            )}
                                          </label>
                                        </div>
                                        <div className="form-switch-cst">
                                        ...
                                        <div className="form-switch-cst">
                                          <input
                                            ...
                                            checked={
                                              jobDetail?.post_production
                                                ?.remove_cables_and_connection_cords ||
                                              false
                                            }
                                            disabled
                                          />
                                        ...
                                        <div className="form-switch-cst">
                                          <input
                                            ...
                                            checked={
                                              jobDetail?.post_production
                                                ?.blurring_personal_photos_names_phone_numbers_or_related ||
                                              false
                                            }
                                            disabled
                                          />
                                          <label
                                            className="text-black fw-bold ms-2 d-inline-block fs-6 text-uppercase"
                                            htmlFor="switch1"
                                          >
                                            {t(
                                              "BLURRING_PERSONALS_PHOTOS_NAMES_PHONE"
                                            )}
                                          </label>
                                        </div>
                                        <div className="form-switch-cst">
                                          <input
                                            ...
                                            checked={
                                              jobDetail?.post_production
                                                ?.remove_electric_outlets_and_distribution_boxes ||
                                              false
                                            }
                                            disabled
                                          />
                                          ...
                                          >
                                            {t(
                                              "REMOVE_ELETRIC_OUTLETS_AND_DISTRIBUTION_BOXES_WATERMARK"
                                            )}
                                          </label>
                                        </div>
                                        <div className="form-switch-cst">
                                        ...
                                        <div className="user-card-details d-flex">
                                          <div className="label-card">
                                            {t(
                                              "ADDITIONAL_PRODUCTION_INFORMATION"
                                            )}
                                          </div>
                                          <div className="card-data">
                                            {jobDetail?.editor_comment_box ||
                                              "-"}
                                          </div>
                                        </div>
                                      </div>
                                      ...
                                      <div className="user-card-details d-flex">
                                        <div className="form-switch-cst">
                                          ...
                                          >
                                            {t(
                                              "LAWN_ENHANCEMENT_REPAIR_OR_REPLACE"
                                            )}{" "}
                                            **
                                          </label>
                                        </div>
                                        ...
                                        <div className="form-switch-cst">
                                          ...
                                          >
                                            {t(
                                              "REMOVE_POOL_OBJECTS_FROM_WATER"
                                            )}{" "}
                                            *
                                          </label>
                                        </div>
                                        ...
                                        <div className="form-switch-cst">
                                          <input
                                            ...
                                            checked={
                                              jobDetail?.post_production
                                                ?.remove_photographer_reflection ||
                                              false
                                            }
                                            disabled
                                          />
                                          ...
                                          >
                                            {t(
                                              "REMOVE_PHOTOGRAPHERS_REFLECTION"
                                            )}{" "}
                                            *
                                          </label>
                                        </div>
                                        ...
                                        <div className="form-switch-cst">
                                          <input
                                            ...
                                            checked={
                                              jobDetail?.post_production
                                                ?.remove_signs_or_builders_labels ||
                                              false
                                            }
                                            disabled
                                          />
                                ...
                                <div
                                  ...
                                  id="toggle-video"
                                  role="tabpanel"
                                  aria-labelledby="toggle-tab-video"
                                >
                                  <div className="card-block">
                                    <div className="d-flex card-middle-row ">
                                      <div className="user-card-details d-flex">
                                        <div className="user-card-details d-flex">
                                          <div className="label-card">
                                            {t("FINAL_LENGTH_SECONDS")}
                                          </div>
                                          <div className="card-data">
                                            {jobDetail?.video_settings
                                              ?.final_length_in_seconds || "-"}
                                          </div>
                                        </div>
                                        <div className="user-card-details d-flex">
                                          <div className="label-card">
                                          ...
                                          <div className="card-data">
                                            {jobDetail?.video_settings?.speed ||
                                              "-"}
                                          </div>
                                        </div>
                                      </div>
                                    ...
                                    <div className="card-middle-row my-4 p-4 video-settings-table">
                                      <div className="w-100  d-flex">
                                        <div className="user-card-details d-flex">
                                          <h4>{t("SPEED_OR_POI")}</h4>
                                        </div>
                                        <div className="user-card-details d-flex">
                                          <h4>{t("REFERENCE_OR_CLIP")}</h4>
                                        </div>
                                        <div className="user-card-details d-flex">
                                          <h4>{t("CAPTION")}</h4>
                                        </div>
                                      ...
                                      {jobDetail?.video_settings?.other_values
                                        ?.length > 0 &&
                                        jobDetail?.video_settings?.other_values?.map(
                                          (video_setting, index) => {
                                            return (
                                              <div
                                                key={index}
                                                className="d-flex w-100 d-flex  video-settings-row  p-3 border-bottom "
                                              >
                                                <div className="user-card-details d-flex ">
                                                  <div className="card-data">
                                                    {video_setting?.speed_or_poi ||
                                                      "-"}
                                                  </div>
                                                </div>
                                                <div className="user-card-details d-flex">
                                                  <div className="card-data">
                                                    {video_setting?.reference_clip ||
                                                      "-"}
                                                  </div>
                                                </div>
                                                <div className="user-card-details d-flex">
                                                  <div className="card-data">
                                                    {video_setting?.caption ||
                                                      "-"}
                                                  </div>
                                                </div>
                                                <div className="user-card-details d-flex">
                                                  <div className="card-data">
                                                    {video_setting?.focus_on ||
                                                      "-"}
                                                  </div>
                                                </div>
                                              </div>
                                ...
                                {imageArrayAdditionalMedia?.length > 0 && (
                                  <div
                                    ...
                                    <div className="card-block">
                                      <div className="job-detail-left position-relative media-gallery">
                                        <h3 className="fs-3 upload-media-title">
                                          {t(
                                            "ADDITIONAL_POST_PRODUCTION_MEDIA"
                                          )}{" "}
                                          ({totalImagesEditorAdditional})
                                        </h3>
                                        <div
                                          onScroll={onHandleInfiniteScroll}
                                          ref={onScrollRef}
                                          className={`without-drop-flex ${
                                            totalImagesEditorAdditional > 12 &&
                                            "img-scroll"
                                          } ${
                                            imageArrayAdditionalMedia?.length > 0 &&
                                            "upload-media-cards"
                                        ...
                                        >
                                          <div
                                            className={`${
                                              imageArrayAdditionalMedia?.length >
                                              0
                                                  ...
                                                  if (
                                                    ...
                                                    return (
                                                      ...
                                                      >
                                                        <Player>
                                                          <source
                                                            src={`${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_extra/${image}`}
                                                          />
                                                          <ControlBar
                                                            autoHide={false}
                                                          />
                                          ...
                                          {imageArrayAdditionalMedia?.length > 0
                                            ? renderDownLoadButton1()
                                            : ""}
                                        </div>
                                        {additionalMediaLoader && (
                          ...
                          <div
                            ...
                            role="tabpanel"
                          >
                            <h3 className="tab-title">{t("RAW_MEDIA")}</h3>
                            {/* tab end */}
                            <div className="card-block">
                              <div className="job-detail-left position-relative">
                                {/* loader */}
                                {imageArrayAssigningMedia?.length === 0 ? (
                                  <div className={`accept-job-text`}>
                                    <h2>{t("NO_MEDIA_AVAILABLE")}</h2>
                                  </div>
                                ) : (
                                  <>
                                    <h3 className="fs-3 upload-media-title">
                                    ...
                                    >
                                      ...
                                      >
                                        {imageArrayAssigningMedia?.length >
                                        0 ? (
                                          imageArrayAssigningMedia?.map(
                                            (image, index) => {
                                              if (
                                                fileExtensionVideo?.includes(
                                                  image
                          ...
                          <div
                            ...
                            id="edited-media-tab-pane"
                            role="tabpanel"
                          >
                            <h3 className="tab-title">{t("EDITED_MEDIA")}</h3>
                            {imageArrayUploadMedia?.length ? (
                              <div className="card-block">
                                <div className="job-detail-left position-relative">
                                  <h3 className="fs-3 upload-media-title">
                                    {imageArrayUploadMedia?.length > 0
                                      ? `${t(
                                          "PHOTO_FROM_PRODUCTION"
                                        )} (${totalImagesEditorUpload})`
                                      : ""}
                                  </h3>
                                  ...
                                  <div
                                    ...
                                    ref={onScrollRef}
                                    className={` without-drop-flex ${
                                      totalImagesEditorUpload > 12 && "img-scroll"
                                    } ${
                                      imageArrayUploadMedia?.length > 0 && "upload-media-cards"
                                    }
                                        }`}
                                  >
                                    <div
                                            ...
                                            if (
                                              ...
                                              return (
                                                ...
                                                >
                                                  <Player>
                                                    <source
                                                      src={`${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_${user_profile_id}/${image}`}
                                                    />
                                                    <ControlBar
                                                      autoHide={false}
                                                    />
                                              ...
                                              return (
                                                ...
                                                >
                                                  <div
                                                    className={handleClassEditedMedia(
                                                      image.split(".")[1]
                                                    )}
                                                  >
                                                    {handleImageEditedMedia(
                                                      image,
                                                      image.split(".")[1]
                                                    )}
                                                  </div>
                              ...
                              <div className="card-block ">
                                <div className="d-flex justify-content-center card-middle-row ">
                                  {(!mediaUploadLoader && imageArrayUploadMediaTemporary?.length === 0) && AvatarPicker()}
                                </div>
                              </div>
                            )}
        ...
        <SliderModal
          ...
          imageUrl={
            scrollTab == 1
              ...
              : scrollTab == 2
              ? `${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_extra/`
              : `${process.env.REACT_APP_AWS_DSP_AWS_URL}job_${job_detail_id}/editor_${user_profile_id}/`
          }
        />
      )}
    </>
